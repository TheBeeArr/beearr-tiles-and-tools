-- Four Directions Tool Edited by BeeArr#8231 // https://github.com/TheBeeArr/beearr-tiles-and-tools/ // GNU General Public License v3 (GPL-3)
...
function getTiles(sx, sy, ex, ey)
    ...
    if map:tileLayer(CURDATA.layer2) and map:tile(tile2) and contains(sx + 2*dx, sy + 2*dy) then
        self.tiles[#self.tiles+1] = { CURDATA.layer2, sx + 2*dx, sy + 2*dy, map:tile(tile2) }
    end
end
