...
function resolveTiles(fence)
	...
	gate_door_w2 = map:tile(FENCE.gate_door_w2),
	...
	gate_door_n2 = map:tile(FENCE.gate_door_n2),
	nw = map:tile(FENCE.nw),
	post = map:tile(FENCE.post),
	ne = map:tile(FENCE.ne),
	se = map:tile(FENCE.se)
    }
end
...
function westEdge(sx, sy, ey)
    ...
    for y=sy,ey,dy do
        if contains(sx, y) then
            local prev = tl:tileAt(sx-1, y)
            local cur = tl:tileAt(sx, y)
            if cur == f.north1 and prev ~= f.ne then
                if prev == f.north1 then
                    self.tiles[#self.tiles+1] = { sx, y, f.ne }
                else
...
function northEdge(sx, sy, ex)
    ...
    for x=sx,ex,dx do
        if contains(x, sy) then
            ...
            local prev = tl:tileAt(x, sy-1)
            if cur == f.west1 then
                if prev == f.west1 then
                    self.tiles[#self.tiles+1] = { x, sy, f.se }
                else
...
function gateTile(x, y, west)
    ...
    local westTile = self:currentLayer():tileAt(x, y-1)
    if westTile == f.gate_door_w then
        return f.gate_door_w2
    end


    local northTile = self:currentLayer():tileAt(x-1, y)
    if northTile == f.gate_door_n then
        return f.gate_door_n2
    end
